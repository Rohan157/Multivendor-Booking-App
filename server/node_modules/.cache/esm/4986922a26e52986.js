let mongoose,bcrypt;_26e‍.x([["default",()=>_26e‍.o]]);_26e‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_26e‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre("save", function (next) {
  ...
  if (user.isModified("password")) {
    return bcrypt.hash(user.password, 12, function (err, hash) {
      if (err) {
        _26e‍.g.console.log("BCRYPT HASH ERR", err);
        return next(err);
      }
      user.password = hash;
      return next();
...
userSchema.methods.comparePassword = function (password, next) {
  bcrypt.compare(password, this.password, function (err, match) {
    if (err) {
      _26e‍.g.console.log("COMPARE PASSWORD ERR", err);
      return next(err, false);
    }
    //if no error we get null
    _26e‍.g.console.log("MATCH PASSWORD", match);
    return next(null, match);
  });
};

_26e‍.d(mongoose.model("User", userSchema));
